...
class AlphaSimulator:
    def __init__(self, max_concurrent: int, alpha_list_file: str):
        ...
        self.alphas_simulated = "simulated_list.csv"
    ...
    def read_alphas_from_csv_in_batches(self):
        ...
        with open(self.alpha_list_file, "r") as file, open(
            temp_file_name, "w", newline=""
        ) as temp_file:
    ...
    def simulate_alpha(self, alpha):
        for attempt in range(36):
            try:
                response = self.session.post(
                    "https://api.worldquantbrain.com/simulations", json=alpha
                )
                response.raise_for_status()
                if "Location" in response.headers:
                    logging.info(
                        f"Alpha location retrieved successfully: {response.headers['Location']}"
                    )
        ...
        logging.error(
            "Simulation request failed after multiple attempts, re-logging in."
        )
        self.session = global_sign_in()
        return None
    ...
    def load_new_alpha_and_simulate(self):
        ...
        if len(self.active_simulations) >= self.max_concurrent:
            logging.info(
                f"Max concurrent simulations reached ({self.max_concurrent}). Waiting 2s..."
            )
            time.sleep(2)
            return
        ...
        if self.sim_queue_ls:
            ...
            logging.info(
                f"Simulating alpha: {alpha['regular']} with settings: {alpha['settings']}"
            )
    ...
    def check_simulation_status(self):
        ...
        with open(self.alphas_simulated, "a+", newline="") as file:
            ...
            for sim_url in self.active_simulations:
                ...
                if result:
                    logging.info(
                        f"Alpha id: {result.get('id')} finished with status: {result.get('status')}"
                    )
                    writer.writerow(
                        {
                            "id": result.get("id"),
    ...
    def finish_active_simulations(self):
        while self.active_simulations:
            self.check_simulation_status()
            logging.info(
                f"Waiting for {len(self.active_simulations)} active simulations to complete..."
            )
            time.sleep(3)
