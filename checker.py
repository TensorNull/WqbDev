import logging
import time
from auth import global_sign_in, setup_logging, retry_request
...
def submit_alpha(alpha_id):
    ...
    while elapsed_time < max_wait_time:
        ...
        if result is None:
            logging.warning(
                f"Get Alpha {alpha_id} Status returned None. Retrying in {check_interval} seconds..."
            )
            ...
            logging.info(
                f"Alpha {alpha_id} status: {result.get('status', 'UNKNOWN')}. Retrying in {check_interval} seconds..."
            )
...
def check_alpha_submission(alpha_id):
    ...
    if retry_request(SESS.patch, base_url, json={"color": "YELLOW"}):
        logging.info(
            f"Mark Alpha {alpha_id} in YELLOW, Reset SELF_CORRELATION to PENDING."
        )
    else:
    ...
    if all(check.get("result") == "PASS" for check in checks):
        if retry_request(SESS.patch, base_url, json={"color": "BLUE"}):
            ...
            logging.warning(
                f"Alpha {alpha_id} Check Submission PASS, But Mark Alpha in BLUE Fail."
            )
        return True
    else:
        if retry_request(SESS.patch, base_url, json={"color": None}):
            ...
            logging.warning(
                f"Alpha {alpha_id} Check Submission FAIL, Clear Alpha Color Fail."
            )
        return False
...
def get_alpha_list():
    ...
    logging.info("+" + "=" * 124 + "+")
    logging.info(
        f"{len(alpha_filtered_list)} Alphas Passed Filter (Sharpe>1.25, Fitness>1, 70>Turnover>1)."
    )
    logging.info(
        f"{len(alpha_nofaile_list)} Alphas IS CHECKS No FAIL, Waiting to Check Submission"
    )
    logging.info("+" + "=" * 124 + "+")
...
def get_checked_alphas():
    ...
    for index, alpha in enumerate(alpha_list, start=1):
        ...
        logging.info(
            f"+[{index:04d}/{total_alphas:04d}] Alpha id: {alpha_id} Start Check Submission"
            + "-" * 72
            + "+"
        )
